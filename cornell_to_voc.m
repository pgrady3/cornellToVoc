...
[~,cmd_output] = system(sprintf('find %s -name "*.png"', target_data_dir));
...
for i = 1:numel(file_names)
    ...
    anno_path = strrep(im_path, 'r.png', 'cpos.txt');
    
    annosCorners = importdata(anno_path);
    annos = [];
    
    
    for bb = 1:size(annosCorners, 1)/4
        k = bb * 4 - 3;
        x = (annosCorners(k + 0, 1) + annosCorners(k + 2, 1)) / 2;        
        y = (annosCorners(k + 0, 2) + annosCorners(k + 2, 2)) / 2;
        
        aSide = annosCorners(k + 3, :) - annosCorners(k + 0, :);
        a = atan(aSide(2) / aSide(1)) * 180 / pi;
        o = norm(annosCorners(k + 3, :) - annosCorners(k + 0, :));
        j = norm(annosCorners(k + 1, :) - annosCorners(k + 0, :));
        
        entry = [x y a o j];
        
        if sum(isnan(entry), 'all') == 0
            annos(end + 1, :) = entry;
        end
    end
    ...
    if visualize
        ...
        for j = 1:size(annos, 1)
           ...
           line(cornersDraw(:, 1), cornersDraw(:, 2));
            ...
            k = j * 4 - 3;
            indRange = k:(k+3)
            rawCorners = annosCorners(indRange, :);
            line(rawCorners(:, 1), rawCorners(:, 2),'Color','red');
        end
    end
